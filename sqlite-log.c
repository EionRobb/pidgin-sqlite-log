...
sqlitelog_list(PurpleLogType type, const char *name, PurpleAccount *account)
{
	...
	if (purple_prefs_get_bool(PREF_GROUPBYDATE))
		sqlite3_prepare(db, "SELECT DISTINCT STRFTIME('%s', DATE(messages.time, 'localtime'), 'utc') FROM messages, logs WHERE messages.log_id=logs.id AND logs.account_id=? AND logs.type=? AND logs.name IN (?,?)", -1, &stmt, NULL);
	else
...
sqlitelog_read (PurpleLog *log, PurpleLogReadFlags *flags)
{
	...
	if (purple_prefs_get_bool(PREF_GROUPBYDATE))
		sqlite3_prepare(db, "SELECT messages.type, messages.who, messages.message, STRFTIME('%s', messages.time) FROM messages, logs WHERE messages.log_id=logs.id AND logs.account_id=? AND logs.type=? AND logs.name IN (?,?) AND DATE(messages.time, 'localtime')=DATE(?, 'unixepoch', 'localtime') ORDER BY messages.time", -1, &stmt, NULL);
	else
